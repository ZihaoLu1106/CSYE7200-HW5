...
object Function {
  ...
  {
    (tryT1: Try[T1], tryT2: Try[T2],tryT3:Try[T3]) =>
      for {
        t1 <- tryT1
        t2 <- tryT2
        t3 <- tryT3
  ...
  {
    (tryT1: Try[T1], tryT2: Try[T2],tryT3:Try[T3],tryT4: Try[T4], tryT5: Try[T5],tryT6:Try[T6],tryT7:Try[T7]) =>
      for {
        t1 <- tryT1
        t2 <- tryT2
        t3 <- tryT3
